...
client.on('message', message => {
    if(message.content.includes('discord.gg')){
                                            if(!message.channel.guild) return message.reply('** من صاحب البوت   **');
...
e.delete();
     message.channel.send('** رائع,قم بكتابة ماذا سوف تفعل في الرتبه**').then(m => {
let chaMd = message.channel.awaitMessages(filter, { max: 1, time: 40000, errors: ['time'] })
.then(co => {
  md = co.first().content
        co.first().delete()
        m.delete();
message.channel.send('**متاكد؟؟ اذا متاكد اكتب نعم و اذا فيه خطا اكتب لا**').then(ms => {
...
client.on('message', message=> {
    ...
    {
    message.reply("https://discordapp.com/api/oauth2/authorize?client_id=484518205495443476&permissions=8&scope=bot || bot by <@252813587188416512> ");
    }
});
